...
export function ArtworksManager({ artworks, collections }: { artworks: Artwork[]; collections: Collection[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await api.post("/api/admin/artworks", body);
      close();
  ...
  async function handleDelete(id: string) {
    ...
    await api.del(`/api/admin/artworks/${id}`);
    window.location.reload();
  }
  ...
  async function handleBatch(status: "IN_SALE" | "SOLD_OUT") {
    ...
    await api.post("/api/admin/artworks/batch-status", { ids: Array.from(selected), status });
