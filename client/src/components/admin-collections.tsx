...
export function CollectionsManager({ collections }: { collections: Collection[] }) {
  ...
  function openEdit(c: Collection) { setEditing(c); setState(undefined); setShowForm(true); }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await api.post("/api/admin/collections", body);
      close();
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await api.del(`/api/admin/collections/${id}`);
      window.location.reload();
    } catch (err) {
