...
export function TestimonialsManager({ testimonials }: { testimonials: Testimonial[] }) {
  ...
  function openNew() { setEditIdx(null); setState(undefined); setShowForm(true); }
  function openEdit(i: number) { setEditIdx(i); setState(undefined); setShowForm(true); }
  ...
  async function handleDelete(i: number) {
    ...
    await api.del(`/api/admin/testimonials/${i}`);
    window.location.reload();
  }
