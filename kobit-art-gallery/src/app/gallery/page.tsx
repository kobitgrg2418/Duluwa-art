...
import { ARTWORKS, COLLECTIONS, collTitle } from "@/lib/data";
...
function GalleryCard({ artwork, index, onOpen }: { artwork: typeof ARTWORKS[0]; index: number; onOpen: () => void }) {
      ...
      >
        ...
        <div className={`gl-card__overlay ${hovered ? "gl-card__overlay--active" : ""}`}>
          <div className="gl-card__info">
            <span className="gl-card__coll">{collTitle(artwork.coll)}</span>
      ...
      <section className="gl-grid-section">
        <div className="wrap">
          <div className="gl-grid">
            {shown.map((a, i) => (
              <GalleryCard key={a.id} artwork={a} index={i} onOpen={() => open(idxOf(a.id))} />
            ))}
          </div>
        </div>
      </section>
